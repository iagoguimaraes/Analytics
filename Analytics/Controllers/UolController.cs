    ...
    public class UolController : ApiController
    {
        ...
        public HttpResponseMessage DashboardBaseAtiva(FormDataCollection form)
        {
            try
            {
                ...
                DateTime data = Convert.ToDateTime(form["dtini"]);
                DataTable segmentacao = JsonConvert.DeserializeObject<DataTable>(form["segmentacao"]);

                string procedure = "sp_dashboard_baseativa";
                if (form["visao"] == "digital")
                    procedure = "sp_dashboard_baseativa";
                if (form["visao"] == "humano")
                    procedure = "sp_dashboard_humano_baseativa";
        ...
        [Autorizar]
        [Gravar]
        public HttpResponseMessage DashboardProjecaoHumano(FormDataCollection form)
        {
            try
            {
                using (SqlHelper sql = new SqlHelper("CUBO_UOL"))
                {
                    DataSet resultado = sql.ExecuteProcedureDataSet("sp_dashboard_humano_projecao");
                    return Request.CreateResponse(HttpStatusCode.OK, resultado);
                }
            }
            catch (Exception e)
