...
{
    [RoutePrefix("api/enel")]
    public class EnelController : ApiController
    {

        #region DIGITAL

        [Route("digital/filtros")]
        [HttpGet]
        [Autorizar]
        [Gravar]
        ...
        public HttpResponseMessage DashboardBaseAtiva(FormDataCollection form)
        {
            try
            {
                ...
                using (SqlHelper sql = new SqlHelper("CUBO_ENEL"))
                {
                    ...
                    DataSet resultado = sql.ExecuteProcedureDataSet("sp_dashboard_baseativa");
                    return Request.CreateResponse(HttpStatusCode.OK, resultado);
                }
            }
            catch (Exception e)
