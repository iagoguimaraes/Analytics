...
namespace Analytics.Controllers
{
    ...
    public class ClaroController : ApiController
    {
        ...
        [Route("dashboard/rcv/pme")]
        [HttpPost]
        [Autorizar]
        [Gravar]
        public HttpResponseMessage DashboardRCVPME(FormDataCollection form)
        {
            try
            {
                DateTime dtini = Convert.ToDateTime(form["dtini"]);
                ...
                using (SqlHelper sql = new SqlHelper("CUBO_CLARO_RCV"))
                {
                    ...
                    DataSet resultado = sql.ExecuteProcedureDataSet("sp_dashboard_pme");
                    return Request.CreateResponse(HttpStatusCode.OK, resultado);
                }
            }
            catch (Exception e)
