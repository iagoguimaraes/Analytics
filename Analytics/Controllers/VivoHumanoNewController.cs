...
namespace Analytics.Controllers
{
    ...
    {        
        ...
        [Route("dashboard/producaodaily")]
        [HttpPost]
        [Autorizar]
        [Gravar]
        public HttpResponseMessage DashboardProducaoDaily(FormDataCollection form)
        {
            try
            {
                DateTime dtini = Convert.ToDateTime(form["dtini"]);
                ...
                using (SqlHelper sql = new SqlHelper("CUBO_VIVO_HUMANO"))
                {
                    ...
                    DataSet resultado = sql.ExecuteProcedureDataSet("sp_dashboard_daily", parametros);
                    return Request.CreateResponse(HttpStatusCode.OK, resultado);
                }
            }
            catch (Exception e)
